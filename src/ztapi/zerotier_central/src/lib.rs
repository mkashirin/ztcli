...
pub mod types {
    ...
    impl ::std::fmt::Display for Member {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            let mut name = self.name.show_or("null").to_string();
            if name.to_string() == "".to_string() {
                name = "unnamed".to_string()
            };
            let node_id = self.node_id.show_or("null");
            let hidden = self.hidden.show_or("null");
            let authorized =
                self.config.as_ref().unwrap().authorized.show_or("null");
            write!(
                f,
    ...
    impl ::std::fmt::Display for Status {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            ...
            let name = user.display_name.show_or("null");
            let email = user.email.show_or("null");
            let id = user.id.show_or("null");

            let api_version = self.api_version.show_or("null");
            let uptime = self.uptime.show_or("null");
